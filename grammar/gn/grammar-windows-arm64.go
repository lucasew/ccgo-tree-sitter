	...
	if *(*uint8)(unsafe.Pointer(valid_symbols + uintptr(STRING_CONTENT))) != 0 {
		...
		for {
			...
			case int32('$'):
				(*(*func(*libc.TLS, uintptr))(unsafe.Pointer(&struct{ uintptr }{(*TSLexer)(unsafe.Pointer(lexer)).Fmark_end})))(tls, lexer)
				advance(tls, lexer)
				if (*TSLexer)(unsafe.Pointer(lexer)).Flookahead == int32('{') || libc.Xisalpha(tls, (*TSLexer)(unsafe.Pointer(lexer)).Flookahead) != 0 || (*TSLexer)(unsafe.Pointer(lexer)).Flookahead == int32('_') {
					(*TSLexer)(unsafe.Pointer(lexer)).Fresult_symbol = uint16(STRING_CONTENT)
					return did_advance
				}
				did_advance = libc.BoolUint8(true1 != 0)
