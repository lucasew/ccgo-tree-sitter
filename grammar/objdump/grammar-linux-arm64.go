...
func is_hexadecimal_character(tls *libc.TLS, character uint8) (r uint8) {
	switch libc.Int32FromUint8(character) {
	case int32('0'):
		fallthrough
	case int32('1'):
		fallthrough
...
func scan_code_identifier(tls *libc.TLS, lexer uintptr) (r uint8) {
	...
	next_token_text = [13]uint8{'(', 'F', 'i', 'l', 'e', 'O', 'f', 'f', 's', 'e', 't', ':'}
	...
	for int32(true1) != 0 {
		...
		if possibly_in_next_hexadecimal_token != 0 {
			if is_hexadecimal_character(tls, libc.Uint8FromInt32((*TSLexer)(unsafe.Pointer(lexer)).Flookahead)) != 0 {
		...
		if !(possibly_in_next_file_offset_token != 0) {
			...
			if (*TSLexer)(unsafe.Pointer(lexer)).Flookahead == libc.Int32FromUint8(next_token_text[offset_counter]) {
