...
func skip_whitespace(tls *libc.TLS, lexer uintptr) {
	...
	for {
		if v10 = (*TSLexer)(unsafe.Pointer(lexer)).Flookahead != int32('\000') && (*TSLexer)(unsafe.Pointer(lexer)).Flookahead != int32('\n'); v10 {
...
func scan_marker(tls *libc.TLS, state uintptr, lexer uintptr) (r uint8) {
	bp := tls.Alloc(512)
	defer tls.Free(512)
	var del_copy, v15 uintptr
	var del_idx, v12 uint32
	var quote int32_t
	var stripping uint8
	var v1, v2, v6, v8, v9 int32
	var v11 bool
	var v4 __darwin_ct_rune_t
	var v5 uint64
	var _ /* delimiter at bp+0 */ [512]int8
	_, _, _, _, _, _, _, _, _, _, _, _, _, _ = del_copy, del_idx, quote, stripping, v1, v11, v12, v15, v2, v4, v5, v6, v8, v9
	...
	del_idx = uint32(1)
	for {
		if v11 = (*TSLexer)(unsafe.Pointer(lexer)).Flookahead != int32('\000'); v11 {
			if quote != 0 {
		...
		if del_idx > uint32(0) {
			v12 = del_idx
			del_idx = del_idx + 1
			(*(*[512]int8)(unsafe.Pointer(bp)))[v12] = int8((*TSLexer)(unsafe.Pointer(lexer)).Flookahead)
		}
		(*(*func(*libc.TLS, uintptr, uint8))(unsafe.Pointer(&struct{ uintptr }{(*TSLexer)(unsafe.Pointer(lexer)).Fadvance})))(tls, lexer, libc.BoolUint8(false1 != 0))
	...
	if (*scanner_state)(unsafe.Pointer(state)).Fheredoc_count == uint32(0) {
		...
		*(*uintptr)(unsafe.Pointer(state + 8)) = del_copy
		...
		if (*scanner_state)(unsafe.Pointer(state)).Fheredoc_count >= uint32(MAX_HEREDOCS) {
			...
			v15 = state + 4
			v12 = *(*uint32)(unsafe.Pointer(v15))
			*(*uint32)(unsafe.Pointer(v15)) = *(*uint32)(unsafe.Pointer(v15)) + 1
			*(*uintptr)(unsafe.Pointer(state + 8 + uintptr(v12)*8)) = del_copy
		}
	}
	(*TSLexer)(unsafe.Pointer(lexer)).Fresult_symbol = uint16(HEREDOC_MARKER)
	return libc.BoolUint8(true1 != 0)
