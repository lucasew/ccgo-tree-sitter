...
func tree_sitter_dockerfile_external_scanner_serialize(tls *libc.TLS, payload uintptr, buffer uintptr) (r uint32) {
	...
	v1 = pos
	pos = pos + 1
	*(*int8)(unsafe.Pointer(buffer + uintptr(v1))) = int8((*scanner_state)(unsafe.Pointer(state)).Fin_heredoc)
	v1 = pos
	pos = pos + 1
	*(*int8)(unsafe.Pointer(buffer + uintptr(v1))) = int8((*scanner_state)(unsafe.Pointer(state)).Fstripping_heredoc)
	i = uint32(0)
	for {
...
func tree_sitter_dockerfile_external_scanner_deserialize(tls *libc.TLS, payload uintptr, buffer uintptr, length uint32) {
	...
	if length == uint32(0) {
		...
		v2 = pos
		pos = pos + 1
		(*scanner_state)(unsafe.Pointer(state)).Fin_heredoc = uint8(libc.BoolInt8(*(*int8)(unsafe.Pointer(buffer + uintptr(v2))) != 0))
		v2 = pos
		pos = pos + 1
		(*scanner_state)(unsafe.Pointer(state)).Fstripping_heredoc = uint8(libc.BoolInt8(*(*int8)(unsafe.Pointer(buffer + uintptr(v2))) != 0))
	...
	for {
		if v2 = (*TSLexer)(unsafe.Pointer(lexer)).Flookahead != int32('\000'); v2 {
			if quote != 0 {
				...
				v1 = libc.BoolInt32(!(libc.Xiswctype(tls, uint16((*TSLexer)(unsafe.Pointer(lexer)).Flookahead), uint16(_SPACE)) != 0))
			}
		}
		if !(v2 && v1 != 0) {
			break
		...
		if del_idx >= uint32(libc.Int32FromInt32(DEL_SPACE)-libc.Int32FromInt32(2)) {
			del_idx = uint32(0)
		}
	}
	if quote != 0 {
